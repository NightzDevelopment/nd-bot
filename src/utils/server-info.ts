...
export async function buildServerInfoEmbed(guild: Guild): Promise<EmbedBuilder> {
  ...
  const verification =
    VERIFICATION_LABELS[guild.verificationLevel] ??
    String(guild.verificationLevel)
  const contentFilter =
    CONTENT_FILTER_LABELS[guild.explicitContentFilter] ??
    String(guild.explicitContentFilter)
  ...
  const embed = ndEmbed().setTitle(guild.name).setThumbnail(guild.iconURL({ size: 256 }))
